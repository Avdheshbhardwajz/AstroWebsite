...
//     const difference = +new Date("2024-12-06T18:00:00") - +new Date();
//     let timeLeft = {
//       days: "00",
//       hours: "00",
//       minutes: "00",
...
//               {[
//                 { icon: Calendar, label: "DATE", value: "6th DEC 2024" },
...
export default function HeroSection({
  ...
  function getTimeLeft() {
    const targetDate = data?.date
      ? new Date(
          `${data.date.replace(/\s/g, "-")}T${data.time.replace(/\s/g, "")}:00`
        )
      : new Date("2024-12-06T18:00:00");
    ...
    <section className="relative bg-white min-h-screen text-black py-16 px-4 overflow-hidden font-poppins">
      ...
      >
        <div className="text-center mb-12">
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold mb-6 leading-tight">
            {data.heading.split(" ").map((word, index) => (
              <span
                key={index}
                className={
                  data.highlightedWords.includes(word) ? "text-[#FF6B2C]" : ""
                }
              >
                {word}{" "}
              </span>
          ...
          <p className="text-lg sm:text-xl md:text-2xl">{data.subHeading}</p>
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                { icon: Video, label: "LOCATION", value: data.location },
              ].map((item, index) => (
                <Card
                  key={index}
                  className="bg-[#062031] border-[#FF6B2C]/30 backdrop-blur-sm"
